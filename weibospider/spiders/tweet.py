...
class TweetSpider(Spider):
    ...
    def start_requests(self):
        ...
        def init_url_by_keywords():
            ...
            keywords = ['%23疫情%23', '%23新冠%23', '%23肺炎%23']
            # keywords = ['%23新冠%23', '%23肺炎%23']
            date_start = datetime.datetime.strptime("2020-01-01", '%Y-%m-%d')
            date_end = datetime.datetime.strptime("2020-04-30", '%Y-%m-%d')
            time_spread = datetime.timedelta(days=1)
            urls = []
            url_format = "https://weibo.cn/search/mblog?hideSearchFrame=&keyword={}" \
                         "&advancedfilter=1&starttime={}&endtime={}&sort=hot&page=1"
            while date_start <= date_end:
                for keyword in keywords:
                    # next_time = date_start + time_spread
                    url = url_format.format(keyword, date_start.strftime("%Y%m%d"), date_start.strftime("%Y%m%d"))
                    urls.append(url)
